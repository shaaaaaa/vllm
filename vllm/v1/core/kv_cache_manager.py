...
from vllm.v1.kv_cache_interface import (
    KVCacheConfig,
    dsa_sparse_decode_d_node_enabled,
)
...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        if (
            ...
            and dsa_sparse_decode_d_node_enabled()
            and num_lookahead_tokens < 1
        ):
            # Full-hit compact bootstrap recomputes only the last prompt token
            # (num_tokens_main_model stays compute_start = hit - 1) but the
            # resident INDEXER must still cover the complete hit prefix. Add
            # one slot without changing the scheduler's computed-token count.

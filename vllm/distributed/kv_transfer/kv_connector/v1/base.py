...
class KVConnectorBase_V1(ABC):
    ...
    def get_request_load_failures(self) -> dict[str, str]:
        """Return request ids whose external lookup/bootstrap has failed.

        Scheduler-side connectors that support a bounded lookup wait override
        this to let the engine finish only the affected requests.  The default
        returns no failures so ordinary connectors incur no extra work.
        """
        return {}

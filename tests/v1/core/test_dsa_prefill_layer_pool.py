...
def test_two_bank_logical_blocks_free_all_physical_banks() -> None:
    ...
    physical_ids = {
        block_id for block in blocks for block_id in block.bank_block_ids or ()
    }
...
def test_latent_and_indexer_share_child_capacity_without_overlap() -> None:
    ...
    latent_physical = {
        block_id for block in latent_blocks for block_id in block.bank_block_ids or ()
    }
    indexer_bundles = {
        child.layout.bundle_id_for_block(DSASharedBlockOwner.INDEXER, block_id)
        for block in indexer_blocks
        for block_id in block.bank_block_ids or ()
    }
    latent_bundles = {
        child.layout.bundle_id_for_block(DSASharedBlockOwner.LATENT, block_id)
...
def test_scheduler_exports_bank_major_physical_ids() -> None:
    ...
    assert blocks.get_allocation_mode() == DSABlockAllocationMode.PREFILL_CHILD

...
@pytest.fixture(params=[False, True], ids=["perf-off", "perf-on"])
def perf_enabled(request):
    return request.param


@pytest.fixture(params=[False, True], ids=["no-spec", "mtp"])
def speculative_config(request):
...
def scheduler(request, perf_enabled, speculative_config):
    namespace = {
        ...
        "time": time,
        "logger": Mock(),
        "SERVING_PERF_ENABLED": perf_enabled,
    }
...
def test_schedule_consumes_pending_bootstrap_once(scheduler, perf_enabled):
    assert scheduler._bootstrap_sample_ready is False
    ...
    for _ in range(2):
        with pytest.raises(_ReachedKVProcessing):
            scheduler.schedule()
        assert scheduler._bootstrap_sample_ready is False
    assert scheduler.schedule.__globals__["logger"].info.call_count == int(perf_enabled)
